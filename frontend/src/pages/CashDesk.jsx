...
export default function CashDesk() {
  ...
  const load = () => {
    ...
    api.get("/orders").then((res) => setOrders(res.data.filter((order) => order.status !== "pago"))).catch(() => {})
  }
  ...
  async function openRegister() {
    try {
      await api.post("/cash/open", { openingAmount: Number(openForm.openingAmount || 0) })
      setError("")
      load()
    } catch (e) {
  ...
  return (
    <AppShell title="Caixa" subtitle="Fluxo financeiro reorganizado no mesmo visual Mercado Urbano.">
      ...
      <div className="grid gap-6 lg:grid-cols-[320px,360px,360px,1fr]">
        ...
        <SectionCard title="Movimentacao" subtitle="Sangria e suprimento com registro direto">
          <div className="space-y-3">
            ...
            <input className="input-soft" placeholder="Observacao" value={movementForm.note} onChange={(e) => setMovementForm({ ...movementForm, note: e.target.value })} />
            <label className="flex items-center gap-2 text-sm text-slate-600">
              <input type="checkbox" checked={movementForm.openCashDrawer} onChange={(e) => setMovementForm({ ...movementForm, openCashDrawer: e.target.checked })} />
              Abrir gaveta
            </label>
        ...
        <SectionCard title="Fechar pedido" subtitle="Selecione a comanda e conclua o pagamento">
          <div className="space-y-3">
            <select className="input-soft" value={closeForm.orderId} onChange={(e) => setCloseForm({ ...closeForm, orderId: e.target.value })}>
              <option value="">Selecione o pedido</option>
              {orders.map((order) => (
                <option key={order.id} value={order.id}>Pedido #{order.id} - Mesa {order.table?.number} - R$ {Number(order.total).toFixed(2)}</option>
              ))}
            </select>
            ...
            <label className="flex items-center gap-2 text-sm text-slate-600">
              <input type="checkbox" checked={closeForm.openCashDrawer} onChange={(e) => setCloseForm({ ...closeForm, openCashDrawer: e.target.checked })} />
              Abrir gaveta
            </label>
        ...
        <div className="space-y-4">
          {registers.map((register) => (
            <div key={register.id} className="card-hover rounded-[28px] border border-slate-200 bg-white p-5 shadow-[0_16px_40px_rgba(20,48,73,0.08)]">
              ...
              <div className="text-sm text-slate-500">Usuario: {register.userName}</div>
              <div className="mt-1 text-sm text-slate-500">Status: {register.status}</div>
              <div className="mt-3 grid gap-2 text-sm text-slate-700">
                <div>Abertura: R$ {Number(register.openingAmount || 0).toFixed(2)}</div>
                <div>Vendas: R$ {Number(register.salesTotal || 0).toFixed(2)}</div>
